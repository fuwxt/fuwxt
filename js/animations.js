(function() {
  ...
  function spawnBurst(x, y) {
    /* Halved (4-6 → 2-3 particles per click). With the WebGL fluid
       now reacting to every pointer event across the whole page,
       a heavy DOM-based ripple swarm on top is overkill — the ink
       splat IS the burst. We keep one or two soft DOM ripples so
       there's still a faint click feedback even if WebGL is paused
       (e.g., reduced-motion users) or unavailable. */
  ...
  function initHeroParticles() {
    ...
    /* Trimmed again: 9 → 5 (desktop), 5 → 3 (mobile). The WebGL
       ink fluid now does the heavy lifting for hero atmosphere;
       these CSS particles are just a tiny extra sparkle. Each
       particle is a positioned + box-shadowed div animating
       transform — cheap individually, but at 9 it added up. */
